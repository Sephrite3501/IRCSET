// server/src/controllers/decisionsController.js
...
/** Middleware: attach DM scope (admin => all categories) */
export async function attachDecisionScope(req, res, next) {
  try {
    ...
    const cq = await appDb.query(
      `SELECT category_id
        ...
        WHERE user_id=$1 AND role_scope='decision_maker'`,
      [uid]
    );
...
export async function makeDecision(req, res) {
  ...
  if (!req.user.dmCategories.includes(sub.category_id)) {
    await logSecurityEvent({
      traceId, actorUserId: req.user.uid, action: 'decision.forbidden', severity: 'warn',
  ...
  try {
    ...
    await appDb.query('ROLLBACK');
    await logSecurityEvent({
      traceId, actorUserId: req.user.uid, action: 'decision.error', severity: 'error',
