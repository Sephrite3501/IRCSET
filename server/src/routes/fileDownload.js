...
function startsWithDir(fullPath, baseDir) {
  ...
  // path.relative won't start with '..' if fullPath is inside baseDir
...
async function canUserSeeFinal(reqUser, submissionId, authorUserId) {
  ...
  if (reqUser.role === 'reviewer') {
    // reviewers: only if assigned to that submission
...
// GET /submissions/:id/final.pdf  (inline by default; append ?dl=1 to force download)
r.get('/submissions/:id/final.pdf', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Require the final to be present and in the right status
    ...
    const stored = path.normalize(sub.final_pdf_path); // we stored a relative path, e.g., uploads/final/xxx.pdf
    const full = path.resolve(process.cwd(), stored);

    // Ensure file is within FINAL_BASE
    if (!startsWithDir(full, FINAL_BASE)) {
      return res.status(400).json({ error: 'Invalid file path' });
    }
    if (!fs.existsSync(full)) {
      // File missing on disk; treat as gone
